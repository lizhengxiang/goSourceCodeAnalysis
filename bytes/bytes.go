...
//explode 将s拆分为UTF-8序列的一个切片，每个Unicode代码点一个（
//仍为//的切片，最多n个字节切片。无效的UTF-8序列被切成单个字节。
...
// If r is utf8.RuneError,
//it returns the first instance of any
//
//IndexRune将 s 解释为一系列UTF-8编码的Unicode代码点。它返回给定符文的 s 中第一个出现的字节索引。如果符文不在 s 中，它会返回-1。如果 r 是utf8. RuneError，
//它将返回任何无效的UTF-8字节序列的第一个实例。
...
// point in common.
//IndexAny 将 s 解释为 UTF-8 编码的 Unicode 代码点序列。它返回字符中任何 Unicode 代码点的 s 中第一次出现的字节索引。
...
func IndexAny(s []byte, chars string) int {
	...
	for i := 0; i < len(s); i += width {
		...
		// r是2到4个字节。使用strings.Index更合理，但由于bytes
		// 包不应导入字符串包，请改用bytealg.IndexString
		// 而且这似乎并没有损失太多性能
...
//字段将s解释为一系列UTF-8编码的代码点。
//它按照unicode.IsSpace的定义，在一个或多个连续的空白字符的每个实例周围分割slice s.IsSpace，返回s的子切片的切片；如果s仅包含空白，则返回
//空的切片。
...
// Join连接s的元素以创建新的字节片。分隔符sep放置在结果切片中的元素之间。
...
//Map 根据映射函数返回字节切片s的所有字符修改后的副本。如果映射返回负值，则字符将从字符串中删除而不会被替换。
...
func Repeat(b []byte, count int) []byte {
	...
	} else if len(b)*count/count != len(b) { // 学到了，这样来处理益出
...
// ToValidUTF8将s视为UTF-8编码的字节，并返回一个副本，其中每个字节的运行
//表示无效的UTF-8被替换的字节替换，该字节可能为空。
...
// asciiSet是一个32字节的值，其中每个位代表该集中存在
//给定的ASCII字符。低16个字节的128位，
//从最低字的最低有效位到最高字的
//高有效位，映射到所有
//128个ASCII字符的全部范围。高16个字节的128位将被清零，
//确保将任何非ASCII字符报告为不在集中。

type asciiSet [8]uint32

// makeASCIISet creates a set of ASCII characters and reports whether all
// characters in chars are ASCII.
// makeASCIISet创建一组ASCII字符并报告char中所有
//字符是否都是ASCII。
...
//如果 old 是空的，它会在切片的开头和每个 UTF-8 序列之后进行匹配，最多可产生  k-1 切片的 k+1 替换。如果 n<0，则替换次数没有限制。
