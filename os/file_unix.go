...
// file is the real(真实) representation(表示) of *File.
// The extra(额外) level of indirection(间接的) ensures that no clients of os
...
type file struct {
	pfd         poll.FD
	name        string
	dirinfo     *dirInfo // nil unless directory being read  //nil，除非正在读取目录
	nonblock    bool     // whether we set nonblocking mode  //是否设置非阻塞模式
...
// seek将下一次在文件上读取或写入的偏移量设置为offset，
//根据whence进行解释：0表示相对于文件的原点，1表示相对于当前的偏移量，2表示相对于文件的偏移量结束。
//返回新的偏移量和错误（如果有）。
