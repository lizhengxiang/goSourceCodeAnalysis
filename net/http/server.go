...
var (
	...
	// body.
	//如果HTTP方法或响应代码不允许正文，则ResponseWriter返回ErrBodyNotAllowed。
	//请求方法或响应状态代码不允许正文
	...
	// effects.
	// ErrHijacked由ResponseWriter返回。当使用Hijacker接口劫持了基础连接时，将写入调用。
	//在被劫持的连接上进行零字节写入将返回ErrHijacked，而不会产生任何其他副作用。
	Err  = errors.New("http: connection has been hijacked")

	// ErrContentLength is returned by ResponseWriter.Write calls
	// when a Handler set a Content-Length response header with a
	// declared size and then attempted to write more bytes than
	// declared.
	// ErrContentLength由ResponseWriter返回。当处理程序设置声明长度的Content-Length响应标头并尝试写入比声明的字节更多的字节时，Write调用。
	ErrContentLength = errors.New("http: wrote more than the declared Content-Length") //http：写的内容多于声明的Content-Length

	// Deprecated: ErrWriteAfterFlush is no longer returned by
	// anything in the net/http package. Callers should not
	// compare errors against this variable.
	//不推荐使用：net / http包中的任何内容都不再返回ErrWriteAfterFlush。调用者不应将错误与此变量进行比较。
...
//处理程序响应HTTP请求。
//ServeHTTP应该将回复标头和数据写入ResponseWriter ，然后返回。返回信号表明请求已完成；
//在使用ServeHTTP 调用之后或同时使用ResponseWriter或从Request.Body读取无效。
//根据HTTP客户端软件，HTTP协议版本以及客户端与Go服务器之间的任何中介，
//在写入ResponseWriter之后，可能无法从Request.Body中读取。谨慎的处理程序应先阅读Request.Body ，然后再进行回复。
//除了读取正文以外，处理程序不应修改提供的Request。
//如果出现ServeHTTP紧急情况，则服务器（ServeHTTP的调用方）假定紧急情况的影响与活动请求无关。
//它恢复紧急状态，将堆栈跟踪记录到服务器错误日志中，然后关闭网络连接或发送HTTP / 2
//RST_STREAM，具体取决于HTTP协议。要中止处理程序，以便客户端看到中断的响应，但是服务器不记录错误，
//出现错误，出现ErrAbortHandler值。
...
// has returned.
//返回Handler.ServeHTTP方法后，可能不使用ResponseWriter。
type ResponseWriter interface {
	// Header returns the header map that will be sent by
	// WriteHeader. The Header map also is the mechanism with which
	// Handlers can set HTTP trailers.
	// Header返回将由 WriteHeader发送的标题映射。 Header映射也是处理程序可以设置HTTP预告片的机制。

	// Changing the header map after a call to WriteHeader (or
	// Write) has no effect unless the modified headers are
	// trailers.
	//除非修改后的标头是尾部，否则在调用WriteHeader（或写）之后更改标头映射无效。
	//
	...
	//
	//设置预告片有两种方法。首选的方法是在标头中预先声明您稍后将发送的预告片，方法是将“ Trailer”标头设置为
	//后继的预告片键的名称。在这种情况下，Header映射的那些键被视为尾部。参见示例。第二种方法，对于直到第一次处理之后才知道的
	//处理程序的尾部键，是在Header映射键之前加上TrailerPrefix
	//常量值。请参阅TrailerPrefix。

	// To suppress automatic response headers (such as "Date"), set
	// their value to nil.
	//若要取消自动响应标头（例如“ Date”），请将其值设置为nil。
	Header() Header
...
// completes.
//注意，即使对于支持Flush的ResponseWriters，
//如果客户端是通过HTTP代理连接的，缓冲的数据也可能直到响应完成才到达客户端。
...
//HTTP / 1.x连接的默认ResponseWriter支持劫持器，但HTTP / 2连接有意不支持。
//ResponseWriter包装器也可能不支持Hijacker。处理程序应该始终在运行时测试此功能。
type Hijacker interface {
	// Hijack lets the caller take over the connection.
	// After a call to Hijack the HTTP server library
	// will not do anything else with the connection.
	//
	// Hijack让呼叫者接管连接。 在调用Hijack之后，HTTP服务器库将对该连接没有任何其他作用。
...
// initALPNRequest是一个HTTP处理程序，用于初始化其* Request中某些未初始化的字段。这种部分初始化的请求来自ALPN协议处理程序。
...
// BaseContext是一个导出但未发布的http.Handler方法，
//被x / net / http2识别以传递上下文； TLSNextProto
//API早于上下文支持，因此我们仅通过提供的唯一接口进行调试。
...
//它仅包含一个字段（并在该字段中包含一个指针字段），因此它适合接口值，而无需额外分配。
...
// tlsRecordHeaderLooksLikeHTTP报告TLS记录头是否看起来是错误的纯文本HTTP请求。
